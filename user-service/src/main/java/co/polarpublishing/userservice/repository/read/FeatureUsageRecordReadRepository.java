...
public interface FeatureUsageRecordReadRepository extends BaseReadonlyRepository<FeatureUsageRecord, FeatureUsageRecord.Id> {

  @Query("select count(fur) from FeatureUsageRecord fur " + 
          "where fur.id.user.id = :userId and fur.id.feature = :feature and fur.id.timestamp >= :from and fur.id.timestamp < :to")
